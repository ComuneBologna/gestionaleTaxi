...
import { DocumentTypes, LicenseeEditOwner, TaxiDriver, TaxiDriverDocument, TaxiDriverVariation, TaxiDriverWrite } from 'app/models/licensees.model';
...
export class OwnerComponent extends BaseComponent implements OnInit {
    ...
    constructor(private _fb: UntypedFormBuilder, private _translateService: TranslateService, private _licenseesService: LicenseesService, private _spinnerService: SpinnerService,
        ...
        private _dialogRef: MatDialogRef<OwnerComponent>, @Inject(MAT_DIALOG_DATA) public data: LicenseeEditOwner) {
        super();
    }
    ...
    ngOnInit(): void {
        ...
        this._licenseesService.getOwner(this.data.licenseeId).subscribe(result => {
    ...
    public save = (): void => {
        if (this.form.isValid()) {
            ...
            if (this.showNote) {
                ...
                obs = this._licenseesService.editOwnerVariation(data, this.data.licenseeId);
            }
            else {
                const data = <TaxiDriverWrite>this.form.value;
                obs = this._licenseesService.saveOwner(data, this.data.isEdit, this.data.licenseeId)
            }
            obs.subscribe(() => {
